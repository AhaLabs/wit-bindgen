...
pub fn generate_json(path: &Path, wit_str: &str) -> Result<()> {
    let mut generator: Box<dyn Generator> = Box::new(JSONSchema::new());
    let mut files = Files::default();
    let path_str = path.to_str().unwrap();
    let (dir, file_stem) = if path_str.ends_with(".json") {
        (
            path.parent().unwrap(),
            path.file_name().unwrap().to_str().unwrap(),
        )
    } else {
        (path, "index.schmea.json")
    };
